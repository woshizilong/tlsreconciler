...
lint-fix: 
	@FILES="$(shell find . -type f -name '*.go' -not -path "./vendor/*")"; goimports -local "github.com/shaj13/tlsreconciler" -w $$FILES
...
release: 
	git clean -df 
	git checkout -- .
	$(shell ./bin/semantic-release --slug shaj13/tlsreconciler) 